...
?	***假如JS脚本文件是在页面之前,就需要将script脚本放到window.onload函数中js代码优先写在后面,后加载后执行***
...
?	方式二:调用**String**()函数,将需要被转换的参数作为参数传递给函数,使用String()函数的时候,对于Number和boolean实际上是调用toString()方法,对于null值和undefined,是将字面量转换成字符串,null转换为"null"
