...
- 抽象类不能被`Final`修饰
- 抽象父类中的构造方法，在子类创建实例的时候会默认Super访问父类的构造方法
- 抽象方法不一定要有抽象方法（适配器模式），但有抽象方法的一定是抽象类
- 抽象方法的子类如果不想实现父类的所有方法，子类必须是一个抽象类，未实现的方法交给子类的子类去实现

![](QQ截图20200824232823.png)
...
补充：JDK动态代理只能代理接口，不能代理类，Spring默认是使用JDK动态代理，需要代理类的时候用cglib/AspectJ代理
